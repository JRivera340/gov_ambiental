...
Última actualización: 2026-07-27 (montaje de AdminDashboard)
...
| Editar punto | GESTOR_AMBIENTAL (+VALIDADOR_AMBIENTAL/ADMIN en hub) | permite editar a validador/admin | solo GESTOR_AMBIENTAL puede editar (`PATCH /puntos/:id`) | PARCIAL | DECIDIDO: replicar comportamiento del hub — permitir VALIDADOR_AMBIENTAL/ADMIN editar (ver Pendiente de replicar, prioridad alta) |
...
Prioridad media:
4. **Recalculo automático de estado de `Proceso`** al aprobar un punto ligado a un proceso (hub: `sorver.controller.ts:486-488`). No visto en `procesos` de gov_ambiental.
...
6. Módulo `files` (subida de acta/fotos a R2) — ver Decisiones abiertas: no implementar hasta fase 2.
