...
const P = (activityId: string) => ({ activityId });

describe('filtrarPuntosAsignados', () => {
  it('incluye los puntos cuyo activityId está en la lista de asignados', () => {
    const out = filtrarPuntosAsignados([P('a1'), P('a2'), P('a3')], ['a1', 'a3']);
    expect(out.map(p => p.activityId)).toEqual(['a1', 'a3']);
  });
  ...
  it('excluye los puntos que no están asignados', () => {
    ...
    expect(out.map(p => p.activityId)).toEqual(['a1']);
    expect(out.some(p => p.activityId === 'a2')).toBe(false);
  });
});
